...
class AlgSolution:
    ...
    def __init__(self):
        ...
        # Fixed forward command for Task A: [v_x, v_y, heading]
        # heading=0 means straight forward (positive x direction)
        # Using higher speed (2.0) for faster traversal
        self.fixed_velocity_commands = torch.tensor(
            [2.0, 0.0, 0.0],  # v_x=2.0 m/s, v_y=0, heading=0 (straight)
            device=self.device,
            dtype=torch.float32,
        ).view(1, 3)
    ...
    def _map_policy_action_to_env_action(self, action_train: torch.Tensor, action_dim: int) -> torch.Tensor:
        """Map training-time 12D leg action to the current env full-body action."""
