...
parser = argparse.ArgumentParser(description="Debug Task A robot motion state.")
...
def main():
    ...
    print(f"start_root_x={start_x:.4f}")
    print("step, elapsed_s, root_x, delta_x, base_lin_vel_x, obs_wheel_vel, action_wheel")
    ...
    for step in range(args_cli.num_steps + 1):
        ...
        if terminated.item() or truncated.item():
            print(f"done at step={step}, total_reward={total_reward:.4f}")
            break

    final_x = robot.data.root_pos_w[0, 0].item()
    print(f"final_root_x={final_x:.4f}")
    print(f"final_delta_x={final_x - start_x:.4f}")
