...
@configclass
class UnitreeB2RoughStraightEnvCfg(UnitreeB2RoughEnvCfg):
    """Rough-terrain curriculum for the Task A straight-line B2/B2W policy."""

    def __post_init__(self):
        super().__post_init__()

        # Task A wants persistent forward motion, not turning-in-place agility.
        ...
        # Make the curriculum focus on terrain difficulty, not command variety.
        self.curriculum.command_levels_lin_vel = None
        self.curriculum.command_levels_ang_vel = None

        # Slightly stronger yaw tracking helps the policy recover after rough terrain knocks it sideways.
        if self.rewards.track_ang_vel_z_exp is not None:
            self.rewards.track_ang_vel_z_exp.weight = 2.0

        # The parent config keeps zero-weight rewards alive for subclasses so they can be edited here.
        # Disable the remaining zero-weight terms now; otherwise wheel-only terms try to resolve empty
        # joint/body regexes on the legged B2 and prevent the environment from starting.
        self.disable_zero_weight_rewards()
