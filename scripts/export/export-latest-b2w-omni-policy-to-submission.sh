...
experiment_root="outputs/rsl_rl/unitree_b2w_rough_omni"
...
if [[ -z "${latest_ckpt}" ]]; then
  ...
  echo "Train the omni policy first: ./scripts/train/train-b2w-rough-omni-from-straight.sh"
  exit 1
fi
...
target_policy="submission/policy_taskd_omni.pt"
...
if [[ -f "${target_policy}" ]]; then
  backup_policy="submission/policy_taskd_omni_before_$(date +%Y%m%d_%H%M%S).pt"
...
echo "submission/policy.pt was NOT touched."
