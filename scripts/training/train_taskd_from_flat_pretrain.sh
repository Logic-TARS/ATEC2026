...
FLAT_EXP_ROOT="logs/rsl_rl/unitree_b2w_taskd_flat_pretrain"
SOURCE_CKPT="$(
...
if [[ -z "$SOURCE_CKPT" || ! -f "$SOURCE_CKPT" ]]; then
  ...
  echo "    ./scripts/train_env.sh --task ATEC-Isaac-Velocity-Flat-TaskF-Unitree-B2W-Piper-v0 --headless --enable_cameras --disable_fabric --num_envs 64 --max_iterations 1000"
  exit 1
fi
