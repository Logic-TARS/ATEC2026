...
# 2. Fall back to submission/policy_taskd_omni.pt
if [[ -z "${ckpt}" ]] && [[ -f "submission/policy_taskd_omni.pt" ]]; then
  ckpt="submission/policy_taskd_omni.pt"
fi
...
if [[ -n "${ckpt}" ]]; then
  ...
  echo "         and submission/policy_taskd_omni.pt is not available."
...
echo "  num_envs       = ${num_envs}"
  echo "  log dir        = outputs/rsl_rl/unitree_b2w_flat_omni"
