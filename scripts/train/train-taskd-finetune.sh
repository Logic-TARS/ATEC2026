...
OMNI_EXP_ROOT="outputs/rsl_rl/unitree_b2w_rough_omni"
if [[ "$STAGE" == "easy" ]]; then
  SOURCE_CKPT="$(
  ...
  SOURCE_CKPT="$(
    find "outputs/rsl_rl/$PREV_EXP" -maxdepth 2 -type f -name 'model_*.pt' \
      | sort -V \
      | tail -1
  )"
fi
...
if [[ -z "$SOURCE_CKPT" || ! -f "$SOURCE_CKPT" ]]; then
  ...
  if [[ -n "$PREV_EXP" ]]; then
    echo "  Expected in: outputs/rsl_rl/$PREV_EXP/"
