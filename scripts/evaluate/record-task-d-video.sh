...
policy_path="${ATEC_POLICY_PATH:-${repo_root}/submission/policy_taskd_omni.pt}"
policy_mode="${ATEC_POLICY_MODE:-b2w_omni16}"
video_output_dir="${ATEC_TASKD_VIDEO_OUTPUT_DIR:-${repo_root}/artifacts/task_d_videos}"
record_video_dir="${repo_root}/outputs/videos/${task_name}/play"
...
if [[ ! -f "${policy_path}" ]]; then
  ...
  echo "  ./scripts/export/export-latest-b2w-omni-policy-to-submission.sh"
  echo
  echo "If you haven't trained it yet, run the training first:"
  echo "  ./scripts/train/train-b2w-rough-omni-from-straight.sh"
  echo
  echo "To use a different policy, set ATEC_POLICY_PATH:"
  echo "  ATEC_POLICY_PATH=submission/policy.pt ATEC_POLICY_MODE=\"\" ATEC_TASKD_COMMAND_MODE=fixed ./record_task_d_b2w_video.sh"
  exit 1
fi
