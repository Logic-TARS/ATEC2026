...
# Usage: ./scripts/evaluate/play-env.sh --task <env_id> [extra_args...]
#
# Examples:
#   ./scripts/evaluate/play-env.sh --task ATEC-Isaac-Velocity-Flat-TaskF-Unitree-B2W-Piper-v0 \
#       --num_envs 1 --checkpoint outputs/rsl_rl/unitree_b2w_flat_omni/<run>/model_1000.pt
#
#   ./scripts/evaluate/play-env.sh --task ATEC-Isaac-Velocity-Flat-Omni-B2W-Piper-v0 \
...
Common extra args (forwarded to tools/atec/rsl_rl/play.py):
...
Examples:
  $(basename "$0") --task ATEC-Isaac-Velocity-Flat-TaskF-Unitree-B2W-Piper-v0 \\
      --num_envs 1 --checkpoint outputs/rsl_rl/unitree_b2w_flat_omni/2025-06-01_12-00-00/model_1000.pt
  ...
  $(basename "$0") --task ATEC-Isaac-Velocity-Rough-Omni-B2W-Piper-v0 \\
      --num_envs 1       --checkpoint outputs/rsl_rl/unitree_b2w_rough_omni/<run>/model_2800.pt \\
...
# Source environment
source "${repo_root}/scripts/env/activate-atec2026-sim.sh"

# Forward all args to the play script
python tools/atec/rsl_rl/play.py "${forwarded_args[@]}"

if [[ "${video}" == true && "${help_requested}" == false ]]; then
    recorded_path="$(
        find "${repo_root}/outputs/rsl_rl" \
    ...
    if [[ -z "${recorded_path}" || ! -f "${recorded_path}" ]]; then
        echo "ERROR: recorded video was not found under ${repo_root}/outputs/rsl_rl."
